...
public class ValidadorCPF {
    ...
    public static boolean validarCpf(String cpf){
        cpf = cpf.replace(".","");
        cpf = cpf.replace("-","");
        //Validar se o tamanho co CPF está de acordo com o esperado
        if (cpf.length() != 11)
            return false;
        //Verificar se o cpf esta demtrp dps invalidos conhecidos
        for (String cpfInvalido: invalidosConhecidos){
            if (cpfInvalido.equals(cpf))
                return false;
        }
        //validação  do primeiro e segundo digiito
        int somatoriaPrimeiroDigito = 0;

        //Validação do primeiro e do segundo digito
        System.out.println(""+validarDigito(multiplicadoresSegundoDigito,cpf));
